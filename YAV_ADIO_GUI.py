...
setting = [1] * 10
if os.path.isfile(os.getcwd() + r'\gain.txt'):
    with open(os.getcwd() + r'\gain.txt', "rt") as file_handler:
        setting = file_handler.read().strip().split('\n')
    setting = map(lambda x: eval(x.split('#')[0].strip()), setting)
...
class YAV_ADIO_GUI:
    ...
    def __init__(self):
        ...
        self.subplot1.set_ylim(bottom=-2.0, top=30.0)
        ...
        self.subplot2.set_ylim(bottom=-100.0, top=5000.0)
    ...
    def animate(self, _index):
        ...
        _voltage = max([_temp_val1, _temp_val2]) * 50.0 * setting[0] / 4096
        _current = abs(_temp_val1 - _temp_val2) * 100000 * setting[1] / 4096 + setting[2]
        if _current < 0.0:
            _current = 0.0
        print "_temp_val1: {}, _temp_val2: {}, abs(A-B)= {}"\
            .format(
                _temp_val1, _temp_val2,
                abs(_temp_val2 - _temp_val1))
        self.fig_time_array = self.array_pop_push(self.fig_time_array, self.fig_time_array[-1] + 1)
        self.fig_voltage1_array = self.array_pop_push(self.fig_voltage1_array, _voltage)
        self.fig_voltage2_array = self.array_pop_push(self.fig_voltage2_array, _current)
        self.l1['text'] = "Voltage: {} V, Current: {} mA, Time: {}".format(
            _voltage, _current, str(datetime.datetime.now() - self.start_time)
        )

        self.subplot2.clear()
        self.subplot1.clear()
        self.subplot2.set_ylim(bottom=-100.0, top=5000.0)
        self.subplot2.plot(self.fig_time_array, self.fig_voltage2_array, color='red',
